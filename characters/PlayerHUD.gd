...
func _on_menu_button_pressed():
	# Проверяем что сцена существует через безопасный метод
	if not _is_scene_tree_available():
		return
	...
	# ← ПРОВЕРКА: БОЙ АКТИВЕН - НЕ ОТКРЫВАЕМ МЕНЮ
	var battle_system = current_tree.get_first_node_in_group("battle_system")
	if is_instance_valid(battle_system) and battle_system.visible:
		return
	
	# ← ПРОВЕРКА: LEVEL UP MENU АКТИВЕН - НЕ ОТКРЫВАЕМ МЕНЮ
	var level_up_menu = current_tree.get_first_node_in_group("level_up_menu")
	if is_instance_valid(level_up_menu) and level_up_menu.visible:
		return
	
	# ← ПРОВЕРКА: ИГРОК МЕРТВ - НЕ ОТКРЫВАЕМ МЕНЮ
	if is_instance_valid(player_stats_instance) and player_stats_instance.current_health <= 0:
		print("❌ Игрок мертв - меню недоступно!")
		return
	
	print("✅ Условия выполнены - открываем меню")
	
	# Ищем меню в сцене
	var menu = current_tree.get_first_node_in_group("pause_menu")
	if is_instance_valid(menu):
		# Отключаем старый сигнал если был подключен
		if menu.menu_closed.is_connected(_on_menu_closed):
...
# НОВАЯ ФУНКЦИЯ: Безопасная проверка доступности дерева сцены
func _is_scene_tree_available() -> bool:
	# Проверяем через Engine.get_main_loop() чтобы избежать ошибок
	if not Engine.get_main_loop() is SceneTree:
		return false
	
	var current_tree = get_tree()
	return current_tree != null and is_instance_valid(current_tree.current_scene)

# НОВАЯ ФУНКЦИЯ: Безопасная установка паузы
func _safe_pause_game():
	if Engine.get_main_loop() is SceneTree:
		var scene_tree = Engine.get_main_loop()
		scene_tree.call_deferred("set", "paused", true)
...
func _on_menu_closed():
	is_menu_open = false
	# Безопасное обновление кнопки меню
	if is_instance_valid(menu_button):
		menu_button.text = "☰"
	# Безопасное снятие паузы - используем try/catch подход
	_safe_unpause_game()
...
func _safe_unpause_game():
	# Пробуем снять паузу, но игнорируем ошибки если сцена уже удалена
	if Engine.get_main_loop() is SceneTree:
		var scene_tree = Engine.get_main_loop()
